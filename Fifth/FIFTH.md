...
Примечание: параболы для построения были вычисленны при помощи numpy, потому что так быстрее. Хотя можно было использовать интерполяцию полиномом Ньютона по трём точкам, но уже пора спать.
