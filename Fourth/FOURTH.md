...
Пусть имеется система линейно независимых на отрезке [a, b] фнукций:
...
Рассматривается следуюшая задача: найти среди всех обобщённых многочленов $P_m(x)$ с $m \le n$ такой, чтобы
...
Отсюда получим истему линейных алгебраический уравнений вида 
...
Если добаить "подлую" точку 2021 года, которая резко обрывает рост, то график с прежними функциями приближения становится нереалистичным.

Поэтому приблицим только функциями $1, x, \exp(x/1000)$:
![](./lsm3.png)
...
Наиболее реалистичный результат получается вычислением МНК при "правильном" выборе базовых функций. Однако, на графике населения есть "подлая" точка за 2021 год, которая выбивается из общей зависимости. То есть скорость прироста населения за последние 10 лет резко снизилась. Такое резконе изменение нельзя предусмотреть математически, если иметь только данные о населении Сингапура. Поэтому ни одна экстраполяция не смогла достаточно близко угадать население Сингапура в 2021 году, основываясь только на данных за предыдущие года. 
